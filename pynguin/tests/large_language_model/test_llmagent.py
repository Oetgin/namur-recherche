...
from pynguin.utils.api_key_resolver import is_api_key_present, require_api_key


@pytest.mark.skipif(
    not is_api_key_present(config.LLMProvider.OPENAI),
    reason="OpenAI API key is not provided in the configuration.",
)
def test_openai_extract_python_code_valid():
...
def test_openai_extract_python_code_multiple_blocks():
    llm_output = "Text\n```python\nprint('Hello')\n```\nMore text\n```python\nprint('World')\n```"
    expected_code = "\nprint('Hello')\n\n\nprint('World')\n"
    model = LLMAgent()
    assert model.extract_python_code_from_llm_output(llm_output) == expected_code
...
def test_require_api_key_missing(monkeypatch):
    ...
    monkeypatch.setattr("pynguin.utils.api_key_resolver.load_dotenv", lambda: None)
...
def test_openai_model_query_success(mocker):
    ...
    mock_response = ("Generated content", usage)

    mock_client = mocker.Mock()
    mock_client.chat.return_value = mock_response
    mocker.patch("pynguin.large_language_model.llmagent.OpenAI", return_value=mock_client)
    model = LLMAgent()
    model.clear_cache()
...
def test_openai_model_query_cache(mocker):
    ...
    usage = {"prompt_tokens": 10, "completion_tokens": 20}
    mock_response = ("Test response", usage)
...
def test_ollama_model_query_success(mocker):
    ...
    # Mock the Ollama client to avoid real API calls
    usage = {"prompt_tokens": 10, "completion_tokens": 20}
    mock_response = ("Generated content", usage)
...
def test_ollama_model_query_cache(mocker):
    ...
    # Mock the Ollama client class to return a mock client
    usage = {"prompt_tokens": 10, "completion_tokens": 20}
    mock_response = ("Test response", usage)
