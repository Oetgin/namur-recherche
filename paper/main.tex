...
\begin{enumerate}
    \item A system to add LLM providers---and a new provider, which allows to run open models locally, reducing the dependency on external providers and improving reproducibility.
    \item A new prompt, which provides more guidance and compresses context to improve the quality of the generated tests.
    \item A system to add imports from the module under test, which allows to resolve objects from the libraries used by the test suite.
    \item Improvements to the deserializer, which allow to parse more statements and assertions.
    \item Other various small improvements, such as handling pybind11 functions, regex improvements, and more.
    \item An evaluation of the impact of these changes on the algorithm performance, in terms of code coverage and execution time.
\end{enumerate}
\todo{Complete contributions}
...
\subsubsection{pybind11 functions}
...
They were selected to have a wide range of complexity and popularity, and to cover different aspects of machine learning, such as neural networks, tabular data generation, and natural language processing.
...
Here are the exact modules we selected:
\begin{itemize}
...
To do so, we ran four different experiments, two for each research question. The first experiment was run with the original prompt, and the second one with our new prompt. The third experiment was run with the original approach, and the fourth one with our new approach.

After each experiment, we collected the code coverage results and execution time, and compared them.

\subsubsection{Containerization}
We deployed our experiments in a compute cluster using Kubernetes. A job was created for all experiments, which created a pod for each experiment. Each pod was running a container, which contained all the dependencies required to run the experiments. The container was built using a Dockerfile, which installed all the dependencies and copied the source code of our project into the container. The Dockerfile was optimized to improve the build time.


\subsubsection{Configuration}
As per the recommendations of Arcuri and Briand~\cite{10.1145/1985793.1985795} on statistical tests for assessing randomized algorithms, we ran each experiment 30 times to account for the randomness of the algorithm. The configuration of the experiments was the default configuration of Pynguin, with the following changes:
\begin{itemize}
    ...
    \item max-plateau-len: 10
    \item create-coverage-report: True
    \item \todo{Complete configuration details}
\end{itemize}
